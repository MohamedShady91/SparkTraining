...
object DataFrames {
  ...
  def main(args: Array[String]) {
    ...
    val lines = spark.sparkContext.textFile("../SparkScala_sundog/fakefriends.csv")
    ...
    println("select the names of all teenagers and add 10 to their ages")
    people.select(people("name"),people("age")+10).filter(people("age") < 21).show()
