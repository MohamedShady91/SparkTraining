...
object PopularMoviesDataSets {
  ...
  def main(args: Array[String]) {
    ...
    */
    ...
    //getting the least popular movies using full query syntax
    moviesDS.printSchema()
    moviesDS.createOrReplaceTempView("movies")
    val leastPopular = spark.sql("select movieID,count(*) as cnt from movies group by movieID order by cnt asc")
    val lstPopular=leastPopular.collect().take(10)
    for (result <- lstPopular) {
      // result is just a Row at this point; we need to cast it back.
      // Each row has movieID, count as above.
      println (names(result(0).asInstanceOf[Int]) + ": " + result(1))
    }
